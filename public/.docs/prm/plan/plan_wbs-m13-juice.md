...
- [ ] 13.1 **Deal/pick/discard motion** — `deal`, `pick`, `unpick`, `discard` animations per the [UX §5 table](../../sdd/software_design_ux.md) (springs, staggers).
- [ ] 13.2 **Toss + Echo** — the 3D flip/settle and single-coin echo re-toss (M12.5 plumbing) tuned to `spring-bouncy`; lands on `Slot.face`.
