...
- **Tier upgrades (M22, applied at score):** the player's purchased pattern upgrades for the matched tier are added to the base — `chips += tierUpgrades[tier].chips`, `mult += tierUpgrades[tier].mult` (a no-tier hand has no upgrades). Applied **before** the boosters, so a `+Mult` charm multiplies the upgraded chips too (Balatro-style compounding). Upgrades persist for the whole run (like charms).
